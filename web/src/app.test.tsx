...
vi.mock('@/services/admin/session', () => ({
...
describe('app getInitialState', () => {
  ...
  it('should read currentUser from local session when not on login page', async () => {
  ...
  it('should redirect to login when no local session exists', async () => {
    const { getInitialState } = await import('./app');
  ...
  it('should include default settings in initial state', async () => {
    const { getInitialState } = await import('./app');
    mockGetCurrentUser.mockReturnValue({ name: 'User' });
  ...
  it('fetchUserInfo should return user data on success', async () => {
    const { getInitialState } = await import('./app');
    mockGetCurrentUser.mockReturnValue({ name: 'Fetched User', access: 'user' });
